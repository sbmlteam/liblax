...
XMLOutputStream::writeChars (const std::string& chars)
{
  for (size_t i=0; i < chars.length(); i++)
  {
    ...
    if ( c == '&' && 
        (LIBLAX_CPP_NAMESPACE ::hasCharacterReference(chars, i) || 
         LIBLAX_CPP_NAMESPACE ::hasPredefinedEntity(chars,i)) )
...
XMLOutputStream::writeValue (const double& value)
{
  ...
  {
    mStream.precision(LIBLAX_DOUBLE_PRECISION);
    mStream <<   value;
  }
                               ...
                               const std::string& programVersion)
{
  ...
  if (programName != "")
  {
    ...
    mStream << " with libLAX version " << getLibLAXDottedVersion();
    mStream << ". -->";
    mStream << endl;
  }
}
